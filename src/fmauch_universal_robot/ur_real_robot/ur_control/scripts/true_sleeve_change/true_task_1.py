...
class TSTPlanner:
    def __init__(self, camera_name, rgb_topic, depth_topic, camera_info_topic):
        ...
        # self.ori_wrench=None
        self.ori_force=None        
        self.ori_torque=None
        # self.cur_wrench=None
        self.cur_force=None
    ...
    def force_callback(self,msg): 
        try:
            ...
            if (self.ori_force is None) and (self.ori_torque is None):
                ...
                self.cur_force = raw_force.tolist()
                self.cur_torque = raw_torque.tolist()
                print ('original wrench collected')
            else:
                cur_force = raw_force-self.ori_force
                cur_torque = raw_torque-self.ori_torque
                self.cur_force = cur_force.tolist()
                self.cur_torque = cur_torque.tolist()
    ...
    def transform_pose(self, src_frame, tgt_frame, pose_pt, ts):
        ...
        try:
            ...
            tf_listener.waitForTransform(tgt_frame, src_frame, ts, rospy.Duration(3))
...
if __name__ == '__main__':
    ...
    try:
        ...
        # pose_target.position.x = 0.190
        # pose_target.position.y = 0.505
        # pose_target.position.z = 0.2
        quat = tf.transformations.quaternion_from_euler(3.14145, 0, -2.9304002)

        pose_target.position.x = 0.224215
        pose_target.position.y = 0.821109
        pose_target.position.z = 0.070040
