...
class TSTPlanner:
    def __init__(self, camera_name, rgb_topic, depth_topic, camera_info_topic):
        ...
        self.sub_extra_rgb = message_filters.Subscriber('/camera2/color/image_raw', Image, queue_size=q)
        self.sub_camera_info = rospy.Subscriber(camera_info_topic, CameraInfo, self.cam_info_cb)
        self.camera_model_ready = False
        self.tss = message_filters.ApproximateTimeSynchronizer([self.sub_rgb, self.sub_depth,self.sub_extra_rgb],
    ...
    def callback(self, rgb_msg, depth_msg, rgb_msg_2):
        try:
            ...
            img_2 = self.bridge.imgmsg_to_cv2(rgb_msg_2, "bgr8")
            ts = rospy.Time.now()
            # rospy.loginfo('receiving image')
            if self.all_infos_lock.acquire():
                # print("lock")
                self.all_infos = {'rgb_img': img, 'depth_img': depth_img, 'rgb_img_2': img_2,
...
if __name__ == '__main__':
    ...
    try:
        ...
        planner = TSTPlanner('camera1', '/camera1/color/image_raw', '/camera1/aligned_depth_to_color/image_raw', '/camera1/color/camera_info')
